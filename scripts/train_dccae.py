...
def train_dccae(config):
    ...
    data = np.load("../data/mnist/wang_mnist.npz")
    ...
    while not success and fail_count < 10:
        try:
            for epoch in range(num_epochs):
                ...
                loss, cca_loss, recon_loss_a, recon_loss_b = model.loss(
                    a_validation, b_validation, x_a_hat, x_b_hat, z_val_a, z_val_b
                )
                if loss < best_loss:
                    best_loss = loss
            ...
            a_acc = calculate_mnist_accuracy(labels[50000:60000], z_train_a, z_val_a)
            b_acc = calculate_mnist_accuracy(labels[50000:60000], z_train_b, z_val_b)

            train.report("accuracy", max(a_acc, b_acc))
            success = True
