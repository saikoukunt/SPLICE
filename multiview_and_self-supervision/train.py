...
def loss_independence(phi_z1, tau_c1, phi_z2, tau_c2):
    ...
    corr = compute_corr(phi_z2, tau_c2)
...
def train(
    ...
    for batch_idx, (x, y, idxes) in enumerate(train_loader_b1):
        ...
        # Using batch1
        match_err, recons_err = loss_matching_recons(
            shared, recons, [x, y], U[idxes, :]
        )
...
def eval_train(model, mmcca2, itr, U, eval_loader, beta, _lambda, device):
    model.eval()
    # mmcca1.eval()
    mmcca2.eval()
    ...
    print(
        "====> Iteration: {} total = {:.4f}, match = {:.4f}, recons = {:.4f}, corr = {:.7f}".format(
            itr,
            match_err + beta * recons_err + _lambda * corr.item(),
            match_err,
            recons_err,
            corr.item(),
        )
