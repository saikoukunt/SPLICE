...
class MMDCCA(nn.Module):
    ...
    def forward(self, x1, x2):
        y1 = self.phi(GradientReversalLayer.apply(x1))
        y2 = self.tau(GradientReversalLayer.apply(x2))
...
class CNNDecoder(nn.Module):
    def __init__(self, z_dim=30, c_dim=2, channels=1):
        super().__init__()
        self.z_dim = z_dim
        self.c_dim = c_dim
        self.pipe = nn.Sequential(
...
class CNNDAE(nn.Module):
    def __init__(self, z_dim=10, c_dim=[0, 2], channels=1):
        super().__init__()
        self.z_dim = z_dim
        self.c_dim = c_dim
        self.num_view = 2
