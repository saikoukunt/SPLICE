...
from splice.base_new import carlosPlus, conv_decoder, conv_encoder, decoder, encoder
        ...
        for epoch in range(epochs):
            ...
            if epoch >= disent_start:
                ...
                for __ in range(msr_iter):
                    cumulative_measurement_loss = 0
                    for batch_start in range(0, a_train.shape[0], batch_size):
                        a_batch = a_train[shuffle_inds][
                            batch_start : batch_start + batch_size
                        ]
                        b_batch = b_train[shuffle_inds][
                            batch_start : batch_start + batch_size
                        ]
                ...
                for batch_start in range(0, a_train.shape[0], batch_size):
                    ...
                    l_disent_a = m_a2b.var(dim=0).sum() / b_train.var(dim=0).sum() if m_a2b is not None else torch.Tensor([0]).to(device)  # type: ignore
                    l_disent_b = m_b2a.var(dim=0).sum() / a_train.var(dim=0).sum() if m_b2a is not None else torch.Tensor([0]).to(device)  # type: ignore
