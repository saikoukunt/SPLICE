...
if __name__ == "__main__":
    ...
    parser.add_argument("--run_id", type=int, default=1, metavar="N", help="run_id")
...
train_loader = DataLoader(
    train_mnist_svhn, batch_size=args.batch_size, shuffle=True, **kwargs
)
test_loader = DataLoader(
    test_mnist_svhn, batch_size=args.batch_size, shuffle=True, **kwargs
)
...
svhn_test_loader = torch.utils.data.DataLoader(
    SVHN(
        "../../data/svhn", split="test", download=True, transform=transforms.ToTensor()
    ),
    batch_size=args.batch_size,
    shuffle=False,
)
...
if CUDA:
    ...
    mnist_net.load_state_dict(
        torch.load("../../data/mnist-svhn/mnist_model.pt", map_location="cpu")
    )
    svhn_net.load_state_dict(
        torch.load("../../data/mnist-svhn/svhn_model.pt", map_location="cpu")
    )
mnist_net.eval()
svhn_net.eval()
...
def get_stat():
    ...
    min_muPrivateA = (
        torch.stack(min_muPrivateA).min(dim=0).values.cpu().detach().numpy()
    )
    max_muPrivateA = (
        torch.stack(max_muPrivateA).max(dim=0).values.cpu().detach().numpy()
    )
    ...
    min_muPrivateB = (
        torch.stack(min_muPrivateB).min(dim=0).values.cpu().detach().numpy()
    )
    max_muPrivateB = (
        torch.stack(max_muPrivateB).max(dim=0).values.cpu().detach().numpy()
    )
    ...
    return (
        [min_muPrivateA, max_muPrivateA],
        [min_muSharedA, max_muSharedA],
        [min_muPrivateB, max_muPrivateB],
        [min_muSharedB, max_muSharedB],
    )
...
def cross_acc_prior():
    ...
    p = probtorch.Trace()
    p.normal(
        loc=torch.zeros((1, args.batch_size, args.n_privateA)),
        scale=torch.ones((1, args.batch_size, args.n_privateA)),
        name="priorA",
    )

    p.normal(
        loc=torch.zeros((1, args.batch_size, args.n_privateB)),
        scale=torch.ones((1, args.batch_size, args.n_privateB)),
        name="priorB",
    )

    p.normal(
        loc=torch.zeros((1, args.batch_size, args.n_shared)),
    ...
    for i, dataT in enumerate(test_loader):
        ...
        if data[0].size()[0] == args.batch_size:
            ...
            ### own recont ###
            reconA = (
                decA.forward2(torch.cat([privA, q["sharedA"].value], -1))
                .view(data[0].size())
                .squeeze(0)
            )
            reconB = decB.forward2(torch.cat([privB, q["sharedB"].value], -1)).squeeze(
                0
            )

            ### cross recont ###
            reconA_cross = (
                decA.forward2(torch.cat([privA, q["sharedB"].value], -1))
                .view(data[0].size())
                .squeeze(0)
            )
            reconB_cross = decB.forward2(
                torch.cat([privB, q["sharedA"].value], -1)
            ).squeeze(0)
            ...
            reconA_poe = (
                decA.forward2(torch.cat([privA, q["poe"].value], -1))
                .view(data[0].size())
                .squeeze(0)
            )
            reconB_poe = decB.forward2(torch.cat([privB, q["poe"].value], -1)).squeeze(
                0
            )
            ...
            reconA_prior = (
                decA.forward2(torch.cat([privA, priorSh], -1))
                .view(data[0].size())
                .squeeze(0)
            )
            ...
            pred_labelA_prior = (
                torch.argmax(pred_labelA_prior, dim=1).cpu().detach().numpy()
            )
            pred_labelB_prior = (
                torch.argmax(pred_labelB_prior, dim=1).cpu().detach().numpy()
            )
            ...
            pred_labelA_poe = (
                torch.argmax(pred_labelA_poe, dim=1).cpu().detach().numpy()
            )
            pred_labelB_poe = (
                torch.argmax(pred_labelB_poe, dim=1).cpu().detach().numpy()
            )
...
def test(encA, decA, encB, decB, epoch):
    ...
    for i, dataT in enumerate(test_loader):
        ...
        if data[0].size()[0] == args.batch_size:
            ...
            pA = decA(
                images1,
                {"sharedA": q["sharedA"], "sharedB": q["sharedB"]},
                q=q,
                num_samples=NUM_SAMPLES,
            )
            pB = decB(
                images2,
                {"sharedB": q["sharedB"], "sharedA": q["sharedA"]},
                q=q,
                num_samples=NUM_SAMPLES,
            )
...
def save_ckpt(e):
    ...
    torch.save(
        encA.state_dict(), "%s/%s-encA_epoch%s.rar" % (args.ckpt_path, MODEL_NAME, e)
    )
    torch.save(
        decA.state_dict(), "%s/%s-decA_epoch%s.rar" % (args.ckpt_path, MODEL_NAME, e)
    )
    torch.save(
        encB.state_dict(), "%s/%s-encB_epoch%s.rar" % (args.ckpt_path, MODEL_NAME, e)
    )
    torch.save(
        decB.state_dict(), "%s/%s-decB_epoch%s.rar" % (args.ckpt_path, MODEL_NAME, e)
    )
...
if args.ckpt_epochs > 0:
    if CUDA:
        encA.load_state_dict(
            torch.load(
                "%s/%s-encA_epoch%s.rar"
                % (args.ckpt_path, MODEL_NAME, args.ckpt_epochs)
            )
        )
        decA.load_state_dict(
            torch.load(
                "%s/%s-decA_epoch%s.rar"
                % (args.ckpt_path, MODEL_NAME, args.ckpt_epochs)
            )
        )
        encB.load_state_dict(
            torch.load(
                "%s/%s-encB_epoch%s.rar"
                % (args.ckpt_path, MODEL_NAME, args.ckpt_epochs)
            )
        )
        decB.load_state_dict(
            torch.load(
                "%s/%s-decB_epoch%s.rar"
                % (args.ckpt_path, MODEL_NAME, args.ckpt_epochs)
            )
        )
    else:
...
for e in range(args.ckpt_epochs, args.epochs):
    ...
    print(
        "[Epoch %d] Train: ELBO %.4e (%ds)" % (e, train_elbo, train_end - train_start)
    )
...
def shared_latent(data_loader, encA, n_samples):
    ...
    for color, i, target_name in zip(colors, target_names, target_names):
        plt.scatter(
            X_r2[:n_samples][labels == i, 0],
            X_r2[:n_samples][labels == i, 1],
            alpha=0.7,
            color=color,
            marker="+",
            s=50,
        ...
        plt.scatter(
            X_r2[n_samples : 2 * n_samples][labels == i, 0],
            X_r2[n_samples : 2 * n_samples][labels == i, 1],
            alpha=0.6,
            color=color,
            s=7,
            linewidths=1,
