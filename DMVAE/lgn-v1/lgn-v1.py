...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--beta1", type=float, default=1, help="multipler for TC [default: 10]"
    )
    parser.add_argument(
        "--beta2", type=float, default=1, help="multipler for TC [default: 10]"
    )
    parser.add_argument(
        "--seed",
        type=int,
...
# MODEL_NAME = (
#     "lgn-v1-run_id%d-privA%02ddim-privB%02ddim-sh%02ddim-lamb_text1_%s-lamb_text2_%s-beta1%s-beta2%s-seed%s-bs%s-wseed%s"
#     % (
#         args.run_id,
#         args.n_privateA,
#         args.n_privateB,
...
# # path parameters
# MODEL_NAME = (
#     "lgn-v1_svhn_cont2-run_id%d-privA%02ddim-privB%02ddim-sh%02ddim-lamb_text1_%s-lamb_text2_%s-beta1%s-beta2%s-seed%s-bs%s-wseed%s"
#     % (
#         args.run_id,
#         args.n_privateA,
#         args.n_privateB,
...
#     )
# )
MODEL_NAME = "lgn-v1_6x200_3000epoch_lambda10_beta2-0.2"
...
BETA1 = (1.0, args.beta1, 0.2)
BETA2 = (1.0, args.beta2, 0.2)
...
X = torch.Tensor(train["responses"]["lgn"][:12096]).to(device)
Y = torch.Tensor(train["responses"]["v1"][:12096]).to(device)

X_val = torch.Tensor(train["responses"]["lgn"][12096:]).to(device)
Y_val = torch.Tensor(train["responses"]["v1"][12096:]).to(device)

dataset = ViewDataset(X, Y)
