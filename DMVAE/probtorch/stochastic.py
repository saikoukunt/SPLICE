        ...
        for n in nodes:
            if n in self._nodes:
                ...
                if (
                    isinstance(node, RandomVariable)
                    and reparameterized
                    and not node.reparameterized
                ):
                    raise ValueError(
                        "All random variables must be sampled by reparameterization."
                    )
    ...
    def log_batch_marginal(self, sample_dim=None, batch_dim=None, nodes=None, bias=1.0):
        ...
        for n in nodes:
            if n in self._nodes:
                ...
                bias_mat[range(batch_size), range(batch_size)] = bias_mat[
                ...
                log_marginal = log_mean_exp(log_pw_joint, 1).transpose(
                    0, batch_dim
                )  # 128,128,1 -128,1-> 1,128
...
def _autogen_trace_methods():
    ...
    for name, obj in _inspect.getmembers(_distributions):
        if (
            ...
            and issubclass(obj, _distributions.Distribution)
            and (obj.has_rsample == True)
        ):
